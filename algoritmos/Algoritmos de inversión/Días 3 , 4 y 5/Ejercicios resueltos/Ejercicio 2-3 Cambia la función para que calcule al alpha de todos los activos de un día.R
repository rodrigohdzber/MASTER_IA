...
profvis({
  ...
  calcular_alpha<-function(dia,rent_activos,rent_DAX,rent_BUND){
    ...
    if (varianza_ind==0){
      ...
      beta[1:length(beta)] <- cov_act_ind[1:length(cov_act_ind)]/varianza_ind
    }
    
    # Calculamos el Alpha del activo. α=Rc-(Rf+β(Rm-Rf))
    alpha<-as.data.frame(matrix(0,nrow=1,ncol=length(rent_activos),byrow=F)) 
    alpha[1:length(activos)]<- rent_activos[dia,1:length(rent_activos)]-(rent_BUND[dia]+beta[1:length(beta)]*(rent_DAX[dia]-rent_BUND[dia]))
  ...
  for (dia in 11:dim(activos)[1]){
    alpha_activos[dia,]<-calcular_alpha(dia,rent_activos,rent_DAX,rent_BUND)
