...
profvis({
  ...
  calcular_mono <- function(){
    ...
    fechas_mono <- sample(fechas_disponibles,2)
    fecha_compra_mono <- sample((ventana_mark+1):(dim(benchmark_global)[1]-min_dias_mono),1)
    fecha_venta_mono <- sample((fecha_compra_mono+min_dias_mono):dim(benchmark_global)[1],1)
    activos_seleccionados_mono = sample(activos_disponibles,num_activos_mono)
    
    activos_mono<-benchmark_global[(fecha_compra_mono-ventana_mark):fecha_compra_mono,activos_seleccionados_mono]
    rent_activos_mono<-rent_activos[(fecha_compra_mono-ventana_mark):fecha_compra_mono,activos_seleccionados_mono]
    
    matriz_correlaciones_mono<-cor(rent_activos_mono)
    matriz_var_covarianzas_mono<-cov(rent_activos_mono) 
    ...
    pesos_mono <- runif(dim(activos_mono)[2]*num_simulaciones_mark_mono, min = 0, max = 100) 
    pesos_mono <- matrix(pesos_mono, nrow=num_simulaciones_mark_mono, ncol=dim(activos_mono)[2], byrow=TRUE) 
    pesos_mono[,1:dim(activos_mono)[2]]<-(pesos_mono[,1:dim(activos_mono)[2]]/rowSums(pesos_mono))
