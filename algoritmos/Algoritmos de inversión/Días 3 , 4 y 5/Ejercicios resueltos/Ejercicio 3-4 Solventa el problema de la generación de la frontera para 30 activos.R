...
profvis({
  ...
  datos<-datos[dim(datos)[1]:(dim(datos)[1]-260),] # Extraemos los datos de un único año.
  datos<-datos[dim(datos)[1]:1,] # El órden de los datos está invertido, tenemos que arreglarlo antes de hacer ningún cálculo
  ...
  rentabilidad_ponderada_riesgo<-log(activos[1,1:length(activos)]/activos[dim(activos)[1],1:length(activos)])/ diag(var(rent_activos))
  
    # Nos quedamos con los N activos que tengan mejor relación rentabilidad riesgo
    n_empresas<-3
    seleccion<-order(rentabilidad_ponderada_riesgo, decreasing = F)[1:n_empresas]
  
    activos<-activos[,seleccion]
    rent_activos<-rent_activos[,seleccion]
