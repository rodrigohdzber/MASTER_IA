# Sacamos 50.000 combinaciones de lotería y analizamos su rendimiento.
...
profvis({
  ...
  sacar_bola <- function(combi,nbola){
    bola <- XXXXXXXXXXXXXXX
    
    # Comprobamos que este número no ha salido ya en la combinación actual
    for (comprobar_bola in 1:nbola){
      if (bola == XXXXXXXXXXXXXXX){
        bola <-XXXXXXXXXXXXXXXXX # Si la bola ya ha salido sacamos otra.
      }
    }
    return(bola)
  }
  ...
  for (combinacion in 1:combinaciones){
    ...
    for (nbola in 1:5){
      combi[nbola]<- XXXXXXXXXXXXXXXXXXX
    }
