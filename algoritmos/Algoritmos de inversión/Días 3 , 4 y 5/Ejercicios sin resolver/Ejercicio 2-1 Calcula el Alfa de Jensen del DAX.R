...
profvis({
  ...
  datos <- read.csv(XXXXXXXXXXXXXXXXX) 
  ...
  for (dia in 2:10){ # Calculamos la rentabilidad de los 10 primeros días (activos, Bund y Dax)
    for (activo in 1:length(activos)){
      rent_activos[dia,activo] <- XXXXXXXXXXXXXXXXXXXXXX
    }
    rent_BUND[dia] <- XXXXXXXXXXXXXXXXXX
    rent_DAX[dia] <- XXXXXXXXXXXXXXXXXXXX
  }
  ...
  for (dia in 11:dim(activos)[1]){
    ...
    for (activo in 1:length(activos)){
      rent_activos[dia,activo] <- XXXXXXXXXXXXXXXXXXXXXXX
    }
    rent_BUND[dia] <- XXXXXXXXXXXXXXXXXXXX
    rent_DAX[dia] <- XXXXXXXXXXXXXXXXXXXX
    
    # Calculamos la varianza del DAX
    varianza_ind<- XXXXXXXXXXXXXXXXXXXXX
    
    for (activo in 1:length(activos)){
      # Calculamos la covarianza entre el activo y el índice
      cov_act_ind <- XXXXXXXXXXXXXXXXXXXXXXXX
      
      # Sacamos la Beta del activo. β=cov(Rc,Rm)/σRm
      if (varianza_ind==0){
        beta <-0
      }else{
        beta <- XXXXXXXXXXXXXXXXXX
      }
      
      # Calculamos el Alpha del activo. α=Rc-(Rf+β(Rm-Rf))
      alpha_activos[dia,activo]<- XXXXXXXXXXXXXXXXXXXXXXXXXX
    }
