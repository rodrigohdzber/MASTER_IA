...
algoritmo_lite<-function(indice,ventana=60){
  ...
  # Obtengo las cookies de su web (curl) 
  curl = getCurlHandle(cookiejar="", verbose = F)
  res = getURI(paste("https://es.finance.yahoo.com/quote/",divisa_a_bajar,"=X?ltr=1", sep=""), curl = curl) 
  ...
  # Construimos la url para bajarnos los datos
  url<-paste("https://query1.finance.yahoo.com/v7/finance/download/",divisa_a_bajar,"=X?period1=",fecha_inicial,"&period2=",fecha_fin,"&interval=1d&events=history&crumb=",crumb, sep="")
  ...
  while(descarga=="Sin datos"){
    ...
    tryCatch(    
      {
        info_divisa = getURLContent(url, curl = curl, verbose = F)
        info_divisa <- read.csv ( textConnection ( info_divisa ) , header = T )
        descarga<-"OK"
      },
      error=function(cond) {
  ...
  # Hay veces que el último día se descarga dos veces en el DF. La segunda vez, la columna de volumen está a cero. Comprobamos si es el caso y eliminamos el último valor.
  if (info_divisa$Date[length(info_divisa$Date)]==info_divisa$Date[length(info_divisa$Date)-1]){
    info_divisa<-info_divisa[1:(dim(info_divisa)[1]-1),]
  }
  
  # Homogeneizamos los datos de la divisa
  info_divisa[,1]<-as.Date(info_divisa[,1])
  datos_homogeneizados<-homogeneizar(info_divisa, ventana)
  
  # No es necesario desmanipular las divisas.
  
  # Nos quedamos únicamente con la columna de cierre ajustado
  nombres_filas<-datos_homogeneizados$Date
  datos_homogeneizados<-as.data.frame(datos_homogeneizados$Adj.Close)
  colnames(datos_homogeneizados)<-divisa
  rownames(datos_homogeneizados)<-nombres_filas
