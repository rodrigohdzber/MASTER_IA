...
algoritmo_lite<-function(XXXXXXXXX,XXXXXXXXXXX){
  ...
  rentabilidades<- XXXXXXXXXXXXXXXXXXX
  ...
  # Buscamos split y contrasplit, de existir, hay que "ajustar" la serie de precios para hacerlos comparables entre sí.
  ...
  # Si la rent log es inferior al -0.69 el precio se ha dividido entre dos en un día, probablemente han realizado un split de la acción.
  
  #             Precio  Rentabilidad  Multiplicador       Precio ajustado
  ...
  for (activo in 2:dim(rentabilidades)[2]){
    ...
    rentabilidades_excesivas<- XXXXXXXXXXXXXXXXXXXXXXX
    
    if (length(rentabilidades_excesivas)>0){ # Si la serie temporal no tiene ningún split o contra split no hay nada que hacer. 
      multiplicador<- XXXXXXXXXXXXXXXXXXXXXXXXXX
      
      for(dia in 2:dim(Adj_close)[1]){ # Recorremos las rentabilidades diarias para construir los multiplicadores.
        if (XXXXXXXXXXXXXXXXXX){ # Han hecho un contra split. Ajustamos el multiplicador.
          multiplicador[dia,1]<- XXXXXXXXXXXXXXXXXXXXXXXXX
        } else if(XXXXXXXXXXXXXXXXXXX){ # Han hecho un split. Ajustamos el multiplicador.
          multiplicador[dia,1]<-XXXXXXXXXXXXXXXXXXXXXX
        } else { # No han hecho nada, el multiplicador no varía.
          multiplicador[dia,1]<- XXXXXXXXXXXXXXXXXXXXXXXX
        }
      }
      # Ajustamos los datos utilizando los multiplicadores. 
      Adj_close[,activo]<-Adj_close[,activo]*XXXXXXXXXXXXX
      High[,activo]<-High[,activo]*XXXXXXXXXXXXXXXX
      Low[,activo]<-Low[,activo]*XXXXXXXXXXXXXX
      Open[,activo]<-Open[,activo]*XXXXXXXXXXXXXXXX
    }
  }
...
for(indice in tickers_de_indices){
  ...
  recomendacion<-algoritmo_lite(XXXXXXXX, XXXXXXXXXX)
}
